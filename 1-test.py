...
class CorrectedPolicyV2(Policy):
    ...
    def _grab_effort(self, o_d: dict[str, npt.NDArray[np.float64]]) -> float:
        ...
        if not self.gasp:
            if np.linalg.norm(pos_curr[:2] - pos_peg[:2]) < 0.04 and (pos_curr[2] - pos_peg[2]) < 0.01: self.gasp = True; return 1.0
            return .0
        return 1.0
...
if __name__ == "__main__":
    ...
    env = ppo_test.make_env(render_mode='human',seed=42, print_flag = True, pos_action_scale = 1)
    ...
    for i in range(num_episodes):
        ...
        while count < 500 and not done:
            ...
            action.set_action(np.hstack((np.zeros(3), 1)))
